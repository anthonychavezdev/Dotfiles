     ...
     (defun acd/copy-file-name-or-path ()
       ...
       (let ((filename (if (derived-mode-p 'dired-mode)
         ...
         (if filename
             (progn
               (kill-new filename)
               (message "Copied '%s' to kill ring" filename))
           (message "buffer not associated to a file" filename))))
   #+end_src
** Version Control
*** Ignore files
    #+begin_src emacs-lisp
...
** Dired
   #+begin_src emacs-lisp
     ...
     (defun acd/copy-file-contents-in-dired ()
       "Copy the contents of the file at point to the kill ring."
       (interactive)
       (let ((file (dired-get-file-for-visit)))
         (with-temp-buffer
           (insert-file-contents file)
           (kill-new (buffer-string)))
         (message "Copied contents of %s" (file-name-nondirectory file))))
   #+end_src
** Eval and replace
   #+begin_src emacs-lisp
...
** Duplicate line
   #+begin_src emacs-lisp
     (defun acd/duplicate-line-dwim (&optional arg)
       "Duplicate the region if active, otherwise the current line, ARG times.
     Does not touch the kill ring."
       (interactive "p")
       (let ((n (or arg 1)))
         (if (use-region-p)
...
** Compile / run programs
   #+begin_src emacs-lisp
     (defun acd/compile-or-recompile ()
       "Compile, or recompile if a compilation buffer already exists."
       (interactive)
...
** Insert date from calendar
   #+begin_src emacs-lisp
     (defvar acd/date-format-options
       ...
       "Date format options for `acd/insert-date-from-cal'.")

     (defun acd/insert-date-from-cal (&optional arg)
       "Insert the calendar date at point into the previously used window.
     With one \\[universal-argument], prompt for a character to append.
     With two \\[universal-argument]s, also prompt for a date format from
     `acd/date-format-options'.  Defaults: format \"%Y-%m-%d\", trailing space."
       (interactive "P")
              ...
              (format (if (equal arg '(16))
                          (let ((completion-extra-properties
                                 (list :annotation-function
                                       (lambda (cand)
                                         (concat "  " (cdr (assoc cand acd/date-format-options)))))))
                            (completing-read "Select a date format: "
                                             acd/date-format-options nil t))
                        "%Y-%m-%d"))
              (char (if arg
                        (let ((input (read-key-sequence
                                      "Character to insert after the date: ")))
                          (if (equal input (kbd "RET")) ?\n (aref input 0)))
                      ?\s))
              (formatted-date (format-time-string
                               format
...
** Milliseconds to date
   #+begin_src emacs-lisp
     (defun acd/milliseconds-to-date (ms)
       "Convert Unix millisecond timestamp MS to a date string; echo and copy it."
       (interactive
        (list (if (use-region-p)
                  (string-to-number
                   (buffer-substring-no-properties (region-beginning) (region-end)))
                (read-number "ms: "))))
       (let ((date (format-time-string "<%F %H:%M>" (/ ms 1000))))
         (kill-new date)
         (message "%s (copied)" date)))
   #+end_src
** Decode JWT tokens
   #+begin_src emacs-lisp
     (defun acd/decode-jwt (&optional jwt)
       "Decode JWT from region, argument, or minibuffer into the *JWT* buffer."
       (interactive)
       (let* ((tok (string-trim
                    (or jwt
                        (if (use-region-p)
              ...
              (parts (split-string tok "\\." t)))
         (unless (>= (length parts) 2)
           (user-error "Doesn't look like a JWT (expected header.claims.signature)"))
         (cl-flet ((decode-part (part)
                     (with-temp-buffer
                       (insert (base64-decode-string part t))
                       (json-pretty-print-buffer)
                       (buffer-string))))
           (with-output-to-temp-buffer "*JWT*"
             (princ (decode-part (nth 0 parts)))
             (princ "\n\n")
             (princ (decode-part (nth 1 parts)))))))
   #+end_src
* Org-mode
** Description
...
*** Code
    #+BEGIN_SRC emacs-lisp
      ;; Elpaca wants a newer version of transient
      ;; This should replace the builtin one
      (use-package transient
        :ensure (:fetcher git :host github :repo "magit/transient")
        :defer t)
...
** Initialize =doom-modeline=
   #+begin_src emacs-lisp
     (use-package doom-modeline
       :ensure t
       :custom ((doom-modeline-support-imenu t))
       :init
...
*** Code
    #+BEGIN_SRC emacs-lisp
      (use-package rainbow-delimiters
        :ensure (:fetcher git :host github :repo "Fanael/rainbow-delimiters")
...
*** Code
    #+begin_src emacs-lisp
      (use-package pdf-tools
        :ensure (:fetcher git :host github :repo "vedang/pdf-tools")
...
*** Code
    #+begin_src emacs-lisp
      (use-package esxml
        :ensure t) ;; required by nov

      (use-package nov
        :ensure (:host "depp.brause.cc"  :repo "nov.el")
        :mode ("\\.epub\\'" . nov-mode))
    #+end_src
** Initialize =elcord=
*** Description
...
#+begin_src emacs-lisp
  (use-package gif-screencast
    :ensure (:fetcher  git :host gitlab :repo "ambrevar/emacs-gif-screencast")
...
#+begin_src emacs-lisp
  (use-package request
    :ensure (:fetcher git :host github :repo "tkf/emacs-request")
    :commands request)
#+end_src
...
*** Code
    #+begin_src emacs-lisp
      ...
      (use-package marginalia
        ...
        ;; The capital `M' indicates that shift should be held
        ;; So hold Shift, Alt, and press `A'
...
*** Code
    #+begin_src emacs-lisp
      ...
      (use-package exec-path-from-shell
        :ensure t
        :custom ((exec-path-from-shell-variables '("PATH" "NIX_PATH"
...
*** Code
    #+begin_src emacs-lisp
      (use-package envrc
        :ensure (:fetcher git :host github :repo "purcell/envrc")
    ...
    #+BEGIN_SRC emacs-lisp
      (use-package yasnippet
        ...
        :custom ((yas-snippet-dirs `(,(concat user-emacs-directory "snippets")
                                     ,(concat elpaca-directory "sources/yasnippet-snippets/snippets"))))
        :config
        (yas-global-mode 1))
      ;; Bundled snippets
...
*** Code
    #+BEGIN_SRC emacs-lisp
      (use-package corfu
       :ensure (:fetcher git :host github :repo "minad/corfu" :files (:defaults "extensions/*")
    ...
    #+begin_src emacs-lisp
      (use-package kind-icon
        ...
        ((kind-icon-default-face 'corfu-default) ; to compute blended backgrounds correctly
         ;; There's an issue where sometimes, the icons appear really large,
         ;; and push items in the popup down, so they're not visible.
         ;; If there aren't enough items to make the popup menu scrollable,
         ;; the items that are pushed down have to be selected in order
         ;; to view them, which is very obnoxious.
         (kind-icons-use-icons nil))
        :config
...
*** Code
    #+begin_src emacs-lisp
      (use-package eglot
        ...
        (add-to-list 'eglot-server-programs '(php-ts-mode . ("laravel-ls")))
...
*** Code
    #+BEGIN_SRC emacs-lisp
      (use-package eglot-java
        :ensure (:fetcher git :host github :repo "yveszoundi/eglot-java")
        :custom
...
*** Code
    #+begin_src emacs-lisp
      (use-package typescript-mode
        :ensure (:fetcher git :host github :repo "emacs-typescript/typescript.el")
...
*** Code
    #+begin_src emacs-lisp
      (use-package groovy-mode
        :ensure (:fetcher git :host github :repo "Groovy-Emacs-Modes/groovy-emacs-modes")
        :mode ("\\.groovy\\'" "\\.gradle\\'"))
    #+end_src
...
*** code
    #+begin_src emacs-lisp
      (use-package fasm-mode
        :ensure (:fetcher git :host github :repo "GabrielFrigo4/fasm-mode")
        :mode "\\.fasm\\'")
    #+end_src
** Initialize =zig-mode=
*** Code
    #+begin_src emacs-lisp
      (use-package zig-ts-mode
        :ensure (:fetcher git :host codeberg :repo "meow_king/zig-ts-mode")
        :mode "\\.zig\\'")
    #+end_src
** Initialize =ocaml=
   #+begin_src emacs-lisp
     (use-package ocaml-ts-mode
       :ensure (:fetcher git :host github :repo "terrateamio/ocaml-ts-mode")
       :mode ("\\.ml\\'" "\\.mli\\'"))
   #+end_src
